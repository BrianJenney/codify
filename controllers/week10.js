...
.controller('Week10Ctrl', ['$scope','$http','$timeout','$window', function($scope,$http, $timeout, $window) {
	...
	return firebase.database().ref('student/' + user.uid + '/week10/').once('value').then(function(snapshot){
			$scope.$apply(function(){
				$scope.week10 = snapshot.val();
				console.log($scope.week10);
				//reinitialize object if null
				//will be null when first called
				if($scope.week10 == null){
					$scope.week10 = {};
				}
			})
		})
	...
	$scope.submitWeek = function(){
		...
		firebase.database().ref('student/' + user.uid + '/week/').set({
			sendGoogleCode: getValue($scope.week10.sendGoogleCode),
			debugPico: getValue($scope.week10.debugPico),
			simpleReplica: getValue($scope.week10.simpleReplica),
			reading: getValue($scope.week10.reading),
			quiz: getValue($scope.week10.quiz)
		})
	}
