...
.controller('WeekSixCtrl', ['$scope','$http','$timeout','$window','chapterService', function($scope,$http, $timeout, $window, chapterService) {
	...
	function getData(){
		firebase.auth().onAuthStateChanged(function(user){
			if(user){
				$scope.user = user;
				chapterService.getUser($scope.user, 'chapter6').then(function(snapshot){
					$scope.$apply(function(){
						$scope.chapter6 = snapshot.val();
					})
				}).then(function(){
	...
	$scope.finishChapter = function(nextChapter){
		...
		$window.location.href = "/#/chapter7.1"; 
	}
