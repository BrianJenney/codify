...
.controller('Week7Ctrl', ['$scope','$http','$timeout','$window', function($scope,$http, $timeout, $window) {
	...
	return firebase.database().ref('student/' + user.uid + '/week7/').once('value').then(function(snapshot){
			$scope.$apply(function(){
				$scope.week7 = snapshot.val();
				console.log($scope.week7);
	...
	$scope.submitWeek = function(){
		...
		firebase.database().ref('student/' + user.uid + '/week/').set({
			sendGoogleCode: getValue($scope.week7.sendGoogleCode),
			debugPico: getValue($scope.week7.debugPico),
			simpleReplica: getValue($scope.week7.simpleReplica),
			reading: getValue($scope.week7.reading),
			quiz: getValue($scope.week7.quiz)
		})
	}
