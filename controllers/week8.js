...
.config(['$routeProvider', function($routeProvider) {
  ...
  .when('/chapter8.1', {
    templateUrl: 'views/chapter8/partials/work1.html',
    controller: 'WeekEightCtrl'
  })

  .when('/chapter8.2', {
    templateUrl: 'views/chapter8/partials/work2.html',
    controller: 'WeekEightCtrl'
  })

  .when('/chapter8.3', {
    templateUrl: 'views/chapter8/partials/work3.html',
    controller: 'WeekEightCtrl'
  })
}])

.controller('WeekEightCtrl', ['$scope','$http','$timeout','$window','chapterService', function($scope,$http, $timeout, $window, chapterService) {
	...
	function getData(){
		firebase.auth().onAuthStateChanged(function(user){
			if(user){
				$scope.user = user;
				chapterService.getUser($scope.user, 'chapter8').then(function(snapshot){
					$scope.$apply(function(){
						$scope.chapter8 = snapshot.val();
					})
				}).then(function(){
	...
	$scope.submitWeek = function(){
		...
		if($scope.chapter8 !== null){
			firebase.database().ref('student/' + user.uid + '/chapter8/').set({
				studentPortfolio: chapterService.getValue($scope.chapter8.studentPortfolio, false),
				intermediateProject: chapterService.getValue($scope.chapter8.intermediateProject, false),
				jsreading: chapterService.getValue($scope.chapter8.jsreading, false)
			});
		}
	...
	$scope.finishChapter = function(nextChapter){
		...
		$window.location.href = "/#/chapter9.1"; 
	}
